"""
Camper 商品图流水线 — step 2 后半步：AI 视角旋转。

前置条件：CAMPER["IMAGE_CUTTER"] 下对应编码的图片已经用
image_pipeline_step2_upload_to_r2.py 上传到 R2（"camper/" 前缀，文件名不变，
{编码}_1.jpg ~ {编码}_5.jpg —— step1 末尾已把原始的 F/C/L/P/T 字母后缀改成了
固定顺序的编号：F->1, C->2, L->3, P->4, T->5）。

Camper 每款商品固定是这 5 张图，不像 ECCO 那样每款图片数量不固定，所以这里直接复用
ops/shoe_angle_ai/run_shoe_angle_rotate_urls.py 的 run_batch()，用法跟
run_shoe_angle_rotate_custom.py 一样 —— 想跑不同参数就复制这个文件改参数，
不用改 run_batch() 本身。

输出目录：CAMPER["IMAGE_ROTATED"]，跟 IMAGE_CUTTER 是两个目录，
...
INPUT_FILE = r"D:\TB\Products\camper\repulibcation\publication_codes.txt"
HEADER_ROWS = 1
CODE_COLUMN_NAME = ""

R2_SHOT_SUBDIR = "camper"
SHOT_SUFFIXES = ["_1", "_2", "_3", "_4", "_5"]

OUTPUT_DIR = str(CAMPER["IMAGE_ROTATED"])
