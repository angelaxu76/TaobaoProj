"""
GEOX 商品图流水线 — step 2 前半步：把裁剪好的图上传到 R2。

跟 image_pipeline_step2_ai_rotate.py 分成两个独立脚本，是因为图片裁剪+改
编号后缀之后，通常需要人工看一眼有没有裁错/缺图，确认没问题再传 R2，不想跟
step1 或 AI 旋转自动连着跑。

上传目录固定读 GEOX["IMAGE_CUTTER"]（跟 step1 的输出、AI 旋转要读的目录
是同一个），只挑 CODE_FILE_PATH 里列的商品编码对应的图，上传后文件名不变，
key 前缀 "geox/"，跟 image_pipeline_step2_ai_rotate.py 里的 R2_SHOT_SUBDIR
保持一致。
...
from config import GEOX, R2_PUBLIC_PREFIX
...
def main():
    ...
    image_dir = Path(GEOX["IMAGE_CUTTER"])
    files = sorted(
        p for p in image_dir.iterdir()
        if p.is_file() and p.stem.split("_")[0] in codes
    )
    if not files:
        print(f"[提示] {image_dir} 下没有找到本批编码对应的图片，请先跑 image_pipeline_step1_download_and_crop.py")
        return
