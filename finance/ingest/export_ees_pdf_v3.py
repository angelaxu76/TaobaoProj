...
def _clean_desc(s: str) -> str:
    ...
    #   "... boots . 40"
    #   "... shoes - 41"
    # 规则：末尾如果是 若干空白 + 标点(-,./_) + 数字(含小数) 就整体砍掉
    t = re.sub(r"[\s\-\.,_/]*(\d+(\.\d+)?)\s*$", "", t)
...
def _get_office_of_exit(summary: dict) -> str:
    """
    从数据里拿出口岸代码：
    - 如果 poe_office 看起来是正常的海关口岸代码（GB + 6 位数字），直接用；
    - 如果是类似 EORI 那种 GB + 12 位数字，就认为有问题，回退成 Heathrow 的 GB000084。
    """
    raw = (summary.get("poe_office") or "").strip()

    # 标准口岸代码一般是 GB + 6 位数字，比如 GB000084
    if re.fullmatch(r"GB\d{6}", raw):
        return raw

    # 否则回退为 Heathrow（你这类 9610 基本都是从这里出）
    return "GB000084"
...
def generate_export_evidence_pdf(invoice_no: str, output_dir: str | None = None):
    ...
    poe_tbl = [
        ...
        ["Office of Exit", _get_office_of_exit(data_summary)],
...
if __name__ == "__main__":
    # 简单 CLI 用于单次测试：python export_ees_pdf_v2.py GB-EMINZORA-251008-1
    if len(sys.argv) >= 2:
        inv = sys.argv[1]
