...
def load_poe_lines(poe_id: str) -> pd.DataFrame:
    ...
    - 对这些商品行要求 purchase_unit_cost_gbp 必须已填写
    ...
    if missing_mask.any():
        missing_rows = df_products[missing_mask]
        example_skuids = missing_rows["skuid"].head(5).tolist()
        raise ValueError(
            f"poe_id = {poe_id} 的商品行中存在未填写采购成本的记录，"
            f"请先通过成本模板补全后再生成发票。\n"
