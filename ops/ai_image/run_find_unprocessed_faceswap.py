...
用法：
  1. SHOT_SUFFIXES 从 _session_config.py 读取，与 run_ai_face_swap.py / run_linkfox_faceswap.py 保持一致。
...
def main(orig_dir=None, faceswap_dir=None, output_excel=None, shot_suffixes=None):
    """路径/后缀参数均可由调用方注入（供 linkfox 等其他渠道复用）。
    不传时从本目录的 _session_config.py 自动读取。"""
    if orig_dir is None:
        sys.path.insert(0, _HERE)
        from _session_config import PERSON_DIR, FACESWAP_DIR as _FD, CODES_EXCEL, SHOT_SUFFIXES
        orig_dir      = str(PERSON_DIR)
        faceswap_dir  = str(_FD)
