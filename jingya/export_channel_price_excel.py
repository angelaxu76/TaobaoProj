...
def export_channel_price_excel(brand: str):
    ...
    df_prices_full = df_prices[["channel_product_id", "product_code", "未税价格", "零售价"]]
...
def export_all_sku_price_excel(brand: str):
    ...
    df_prices = df_grouped.join(df_grouped.apply(calculate_prices, axis=1))
    df_prices["product_code"] = df_prices["product_code"].astype(str).str.strip().str.upper()
    df_prices_filtered = df_prices[~df_prices["product_code"].isin(excluded_names)]

    df_sku = df_prices_filtered[["product_code", "零售价"]]
