...
def barbour_database_import_pipleline():
    ...
    print("步骤 5：为发布的商品选择合适的供应商，比如库存充足且价格低的")
    #Step 4: TODO 根据发布文件填充barbour 鲸芽的map表
    # fill_supplier_map()

    print("     步骤 5.1：为重点商品强制指定供应商覆盖")
    # xlsx_path = r"D:\TB\Products\barbour\publication\barbour_low_stock_supplier_reassignment_suggestions.xlsx"
    # apply_barbour_supplier_overrides(xlsx_path,dry_run=True)

    print("     步骤 5.2：生产发布商品的供应商报表")
    ...
    print("     步骤 5.3：为发布商品低库存的商品重新分配供货商")
    # 先 dry-run 看建议
    ...
    print("步骤 6：将barbour inventory清空，并重新填充已发布商品信息，通过jingya id导出的excel文件")
